...
- 对话页每轮带**可折叠的工具调用轨迹**：阶段标记（理解偏好 → 评分 → 写推荐语）、推荐语**逐张进度（1/3 … 3/3，流式）**、每次 LLM 调用的 token 用量与费用、本轮汇总。
...
| 首次同步慢 | 正常：受 Steam 商店接口限速（36/60 次每分双通道），进度条与 ETA 实时可见；之后增量秒级 |
...
```bash
cargo test   # 77 项：VDF/manifest 解析、账号选择（含 Timestamp 兜底）、存储、key 脱敏（Steam/LLM 双侧）、
             # 代理归一化、成就分类与 LLM 输出解析（含流式 SSE 帧）、四维合成、三源定位映射、
             # 软件类目判定、.env 写入与密钥校验、同步/LLM 错误友好映射、抽样与探索位、
             # 计费（缓存命中分档/价格覆盖/时段/预算）、剧情覆盖率通关、玩家标注覆盖、引导状态等
```
...
src/
  ...
  profiler.rs     # 成就类型在线分析（C4）+ 四维画像 + 注水提议 + 识别异常
